...
# This build script has some sort of reversal... don't know why I tried this. I believe it was to check if an extra
# cell was viable, at Jacques' request.

EXPORT_CONTOURS_TO = "export_contours/contours_RS_P%02d_%d_nuc_20211115"
...
Z_SCALE = 0.1
Z_SCALE_LIST = [None, 0.1, 0.1, 0.1, 0.2, 0.1, 0.1, 0.2]
...
def main(output_folder, annotation_folder, probe_info=None, npts=21, padding=20, mirror_x=0, mirror_y=0,
         title='title', comment='comment', doPlot=False, z_scale=Z_SCALE, options={}):
    ...
    if 'reversal' in options:
        print('Considering reversal')
        if options['reversal']['cyto']: ee = contour.reverse_layers(ee, top=TOP_LAYER)
        if options['reversal']['nucleus']:
            print('old levels:', list(ne.keys()))
            ne = contour.reverse_layers(ne, top=TOP_LAYER)
            print('new levels:', list(ne.keys()))
...
if __name__ == '__main__':
    ...
    # for _ in [3, 6]:  # A redo of 3, 6
    for _ in [4,]:  # Special reversals for RS4
        ...
        for pn in range(len(all_padding)):
            ...
            main(output_folder=EXPORT_CONTOURS_TO % (padding, _),
                 ...
                 doPlot=DO_PLOT,
                 options={'reversal': {'cyto': False, 'nucleus': True, 'other': True}}
