...
EXPORT_CONTOURS_TO = "export_contours/contours_huvec_P%02d_%d_redo"
EXPORT_CONTOURS_TO = "export_contours/temp/contours_huvec_P%02d_%d_temp"
ANNOTATION_FOLDER = 'HUVEC Cell %d/annotation'
TITLE = 'HUVEC_%d'
COMMENT = "AFM-C HUVEC Cell #%d"
...
def main(output_folder, annotation_folder, probe_info=None, npts=21, padding=20, mirror_x=0, mirror_y=0,
         title='title', comment='comment', doPlot=False):
    ...
    os.makedirs(os.path.join(output_folder,'dumped'),exist_ok=True)
    ...
    dumpable = {
        'cyto_meta': cyto_meta,
        'nuc_meta': nuc_meta,
        'nucleus_contour':nucleus_contour,
        'cyto_contour': cyto_contour,
        'scaling':(0.03237/sf, 0.03237/sf, Z_SCALE),
        'ee': ee,
        'ne': ne,

    }

    np.save(os.path.join(output_folder,'dumped','to_dump.npy'), dumpable)
    ...
    np.save(os.path.join(output_folder,'dumped', 'extra.npy'), data)
...
if __name__ == '__main__':
    ...
    all_xy_mirror = [[None, 0, 0, 0, 0, 0, 0, 0, 0], [None, 1, 1, 0, 0, 0, 0, 0, 0]]

    all_padding = [0, 10, 20, 30, 40, 80]
    for _ in [1]: #
