...
if __name__ == '__main__':
    ...
    all_tips = np.load(os.path.join(coord_folder, 'centered_all.npy'), allow_pickle=True)
    ...
    for _ in [1,2,4,5,6,7,8]: # [1,]: # bad: 4 (no files), 6, 7 (no files)
        ...
        for pn in range(len(all_padding)):
            ...
            main(output_folder="export_contours/contours_hgps1_P%02d_%d" % (padding, _),
                 annotation_folder=os.path.join(base_folder,'HGPS %d/annotation' % _),
                 probe_info=[all_tips[0][_][:2], all_tips[1][_][:2]],
                 mirror_x=mirror_x,
                 mirror_y=mirror_y,
                 title='HGPS1_%d' % _,
                 comment="AFM-C HGPS Cell #%d" % _,
                 padding=padding,
                 doPlot=True,
