...
def export_sr_ome(target_dir, source_folder, fname, fname_stub='Intensity_Z', as_colors=None, n_channels=3, keep3=[0,1,2]):
...
def export_set_for_annotation(cs1, tag=None, use_log=False, do_show=False):
    if tag is None:
    ...
    for f in range(int(ranged[2]), int(ranged[5])):
        ...
        if use_log:
            ...
            plt.imshow(sliced)
        plt.title(tag + ' ' + str(f))
        figpath = os.path.join(annotation_folder, tag + '_%02d.png' % f)
