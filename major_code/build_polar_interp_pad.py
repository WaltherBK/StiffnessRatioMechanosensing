...
if __name__ == '__main__':
    ...
    all_tips = np.load(os.path.join('datasets/SR_HUVEC1/Coordinate Overlays/', 'centered_all.npy'), allow_pickle=True)
    print('Loaded tips:', all_tips)

    all_xy_mirror = [[None, 0, 0, 0, 0, 0, 0, 0], [None, 0, 0, 0, 0, 0, 0, 0]]

    all_padding = [0,  80] # 10, 20, 30, 40,
    for _ in range(1,1+1): # [1,]: # bad: 4 (no files), 6, 7 (no files)
        ...
        for pn in range(len(all_padding)):
            ...
            main(output_folder="export_contours/newprobe_HUVEC1_I%d_Trash%02d" % (_, padding),
                 annotation_folder='datasets/SR_HUVEC1/HUVEC Cell %d/annotation' % _,
                 probe_info=[all_tips[0][_][:2], all_tips[1][_][:2]],
                 mirror_x=mirror_x,
                 mirror_y=mirror_y,
                 title='HUVECC_%d' % _,
                 comment="AFM-C HUVEC 20200626 Group #1, Rescaled Probes, Cell #%d" % _,
                 padding=padding,
                 doPlot=True,
